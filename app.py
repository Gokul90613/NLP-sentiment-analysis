...
def is_language_valid(text, language):
    """Check if the text matches the selected language."""
    try:
        detected_language = detect(text)
        if language == "hindi" and detected_language != "hi":
            return False
        elif language == "english" and detected_language != "en":
            return False
        return True
    except Exception:
        return False
...
def predict():
    try:
        ...
        if not is_language_valid(text, language):
